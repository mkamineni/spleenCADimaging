...
def format_data_helper(sub_path, cnt, num_of_digits, img_basenames, nnunet_folder, json_file):
    img_list = []
    conversion_map = []
    ...
    return img_list, conversion_map
...
def format_data(nifti_folder, nnunet_folder, json_file, num_subjects, start_idx):
    ...
    for sub_path in subjects:
        ...
        args.append((sub_path, cnt, num_of_digits, img_basenames, nnunet_folder, json_file))
        cnt+=1

    pool =  mp.Pool(mp.cpu_count())
    res = pool.starmap(format_data_helper, args)

    img_list, conversion_map = zip(*res)
