...
def stitch(zip_file, subject_dir, subject_id, tool):
    ...
    if not is_stitching_correct(subject_dir):
        ...
        #afterzip = time()
        #print(f'Afterzip took {afterzip - stitch_start} seconds!')
        
        # this step is taking a really long time! 11 sec
        dicom2nifti.convert_directory(dicom_dir, subject_dir)
        shutil.rmtree(dicom_dir)

        #dicom2nifti_end = time()
        #print(f'Dicome2nifti took {dicom2nifti_end - afterzip} seconds!')
        ...
        #before_loop_end = time()
        #print(f'Before loop took {before_loop_end - dicom2nifti_end} seconds!')
        
        # this for loop takes 24 sec
        ...
        if len(nii_files) >= 24:
            ...
            #stitch_loop_end = time()
            #print(f'Stitch loop took {stitch_loop_end - before_loop_end} seconds!')
...
def main():
    ...
    #Meg: getting rid of this code because we always analyze all zip images
    #if start_idx < 0:
    #    start_idx = 0
    ...
    #end = time()
    #print(f'Up until getting stitching took {end - start} seconds!')
    ...
    for f in zip_files:
        ...
        args.append((f, subject_dir, subject_id, tool))
        #stitch(f, subject_dir, subject_id, tool)

    pool.starmap(stitch, args)
